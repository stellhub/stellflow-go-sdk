...
- Producer 默认在 Metadata 返回空分区或未知 topic 时通过 `CreateTopic` 自动创建 topic，刷新 Metadata 后再继续发送。
- 自动创建 topic 可通过 `producer.Options.DisableAutoCreateTopics` 关闭，默认创建分区数由 `AutoCreateTopicPartitionCount` 控制，默认值为 `2`。
...
- `CreateTopic(ctx, topic, partitionCount)`：通过服务端 `CreateTopic` 数据面管理协议创建指定分区数的 topic。
- `CreateTopicIfAbsent(ctx, topic, partitionCount)`：先查询 Metadata，topic 不存在时再创建，用于业务启动阶段显式补齐 topic。
- `ListOffsets(ctx, requests)`：支持 earliest、latest 和 timestamp offset 查询。

Topic 删除、分区调整、Broker 健康检查和 Broker 下线等管理类 API 应在对应请求 / 响应 codec 完成后再暴露，不要先提供伪接口。
...
func main() {
	...
	factory, err := stellflow.NewClientFactory(stellflow.Options{
		...
		Producer: producer.Options{
			AutoCreateTopicPartitionCount: 2,
		},
	})
	if err != nil {
		log.Fatal(err)
